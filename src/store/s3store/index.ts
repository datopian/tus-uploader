// Adopted from https://github.com/tus/tus-node-server/tree/main/packages/s3-store
...
const log = debug('tus-uploader:stores:s3store')
...
export class S3Store extends DataStore {
  ...
  public async create(upload: Upload) {
    ...
    const request: AWS.CreateMultipartUploadCommandInput = {
      ...
      Metadata: {
        'tus-version': TUS_RESUMABLE,
        'meta-hash': upload.metadata?.checksum as string
      },
    }
  ...
  public async getUpload(id: string): Promise<Upload> {
    ...
    try {
      ...
      if ((error as any).Code === 'NoSuchUpload' || (error as any).Code === 'NoSuchKey') { 
        return new Upload({
          ...metadata.file,
  ...
  public async remove(id: string): Promise<void> {
    ...
    if ((error as any)?.code && ['NotFound', 'NoSuchKey', 'NoSuchUpload'].includes((error as any).code)) {
